...
function epsilon_aut_size(A::AbstractMatrix{<:Real}) # This matrix may be whatever category of matrix, but it should be square and symmetric.
...
g = watts_strogatz(11, 4, 0.0, seed=123) # Generate a Ring
...
degrees = degree(g)
CSV.write("zs/deg.csv", DataFrame(degree=degrees)) # Save the degree distribution

A = adjacency_matrix(g) # Get the adjacency matrix of the graph

epsilon_bins, automorphism_size, histogram, first_bin = epsilon_aut_size(A) # Compute the epsilon vector and automorphism size

data = hcat(epsilon_bins, automorphism_size) # Combine the epsilon bins and automorphism sizes into a single matrix
writedlm("zs/rem.csv", data, ',') # Write the data to a CSV file
